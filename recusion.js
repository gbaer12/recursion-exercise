/** Product:
 *
 * calculate the produc of an array of numbers.
 */
...
/** binarySearch: given a sorted array of numbers, and a value, return the index of the value(or -1 if the val is not present). */
...
function balancedBrackets(str) {
  function balancedBrackets(str, index, openCount) {
    if (index === str.length) {
      return openCount === 0;
    }

    if (str[index] === "(") {
      return balancedBrackets(str, index + 1, openCount + 1);
    } else if (str[index] === ")") {
      if (openCount === 0) {
      ...
      return balancedBrackets(str, index + 1, openCount);
    }
  }

  return balancedBrackets(str, 0, 0);
}
